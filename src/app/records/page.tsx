...
export default function RecordTab() {
    ...
    return (
        <div className="space-y-3">
            ...
            {showAdd && <AddRecord onSuccess={() => { setShowAdd(false); refetch(); }} />}
            {loading && <div className="text-sm text-gray-500 animate-pulse px-1">Loading...</div>}

            {records.map((r) => (
                <RecordComponent key={r.id} record={r} onSuccess={()=>refetch()}/>
            ))}
...
function AddRecord({ onSuccess }: { onSuccess: () => void }) {
...
function RecordComponent({ record, onSuccess }: { record: Record; onSuccess: () => void }) {
    ...
    const handleUpdate = async () => {
        await fetch(`/api/record/${record.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            body: JSON.stringify({
